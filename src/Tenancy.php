...
class Tenancy
{
    ...
    public function end(): void
    {
        ...
        if (! $this->initialized) {
            // Octane hardening: still null any leftover tenant ref. Under long-running
            // workers a previous request may have set $tenant without flipping
            // $initialized true (e.g. an exception between InitializingTenancy and
            // TenancyInitialized). Returning here without clearing would leak the
            // tenant reference into the next request.
            $this->tenant = null;
            return;
        }
