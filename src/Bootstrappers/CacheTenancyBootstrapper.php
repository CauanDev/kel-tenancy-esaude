...
class CacheTenancyBootstrapper implements TenancyBootstrapper
{
    ...
    public function bootstrap(Tenant $tenant)
    {
        ...
        // Octane hardening: use instance() instead of extend(). extend() APPENDS
        // a closure to $extenders['cache'] in the container — under a long-running
        // Octane worker this leaks one closure per bootstrap/revert pair and
        // degrades performance of every app('cache') resolution. instance() is
        // an atomic replace.
        $this->originalCache = $this->originalCache ?? $this->app['cache'];
        $this->app->forgetExtenders('cache');
        $this->app->instance('cache', new TenantCacheManager($this->app));
    }
    ...
    public function revert()
    {
        ...
        $this->app->forgetExtenders('cache');
        $this->app->instance('cache', $this->originalCache);
